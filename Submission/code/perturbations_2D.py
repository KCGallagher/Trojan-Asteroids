...
wander_data = wander.rand_sample(max_pertubation_size=0.01, samples=100)
...
grid_size = 0.04  # Range sampled in AU/(AU/year)
sampling_points = 16
...
# This code can also be used for position perturbations by changing the type here
...
# Change these labels when plotting position space instead of velocity space

plt.savefig("velocitymesh.png")
plt.show()
...
# Combinations of these initial conditions used to compare orbits
...
# Note that relevant peaks may not appear on plot selection if other parameters are used

# DEVIATION FROM LAGRANGE POINT

plt.rcParams.update({"font.size": 15})
# larger font for these plots as they will be side by side
